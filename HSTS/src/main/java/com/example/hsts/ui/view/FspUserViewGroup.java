...
public class FspUserViewGroup extends ViewGroup {
        ...
        if (childrenCount <= 2) { // <=2
            for (int i = 0; i < childrenCount; i++) {
                ...
                if (child.isSelected()) {
                    ...
                    setChildDoubleClickListener(0,0, 900, 600, child);
                    if (i == 0) {
                        child.layout(0,0, 900, 600);
                    } else {
    ...
    public void onEventRemoteLeaveVideo(FspEvents.RemoteUserEvent event) {
        FspManager fspManager = FspManager.getInstance();
        int childCount = getChildCount();
        for (int i = 0; i < childCount; i++) {
            FspUserView child = (FspUserView) getChildAt(i);
    ...
    public void onEventRemoteVideoAndAudio() {
        ...
        //Logger.e("videoView222 audiocount == %d", fsm.getRemoteAudios());
