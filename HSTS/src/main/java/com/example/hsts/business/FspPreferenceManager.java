...
import com.example.hsts.utils.VoiceVariantUtils;
...
public class FspPreferenceManager {
    ...
    public FspPreferenceManager(Context context) {
        m_sharedPreferences = PreferenceManager.getDefaultSharedPreferences(context);
        m_editor = m_sharedPreferences.edit();
    }

    public static FspPreferenceManager getInstance(Context context) {
        if (s_instance == null) {
            synchronized (FspPreferenceManager.class) {
                if (s_instance == null) {
                    s_instance = new FspPreferenceManager(context);
                }
            }
        }
        return s_instance;
