...
public class ImageUtil {
    ...
    public static Rect getRect(FaceInfo faceDetails){
        if (faceDetails == null){
            return null;
        }
        //return new Rect(faceDetails.x , faceDetails.y , faceDetails.x + faceDetails.width , faceDetails.y + faceDetails.height);
        return faceDetails.getRect();
    }
    ...
    public static byte[] getPixelsBGR(Bitmap image) {
        ...
        image.copyPixelsToBuffer(buffer); // Move the byte data to the buffer
        byte[] temp = buffer.array(); // Get the underlying array containing the data.
        byte[] pixels = new byte[image.getWidth()*image.getHeight()*3]; // Allocate for BGR
        // Copy pixels into place
        for (int i = 0; i < (temp.length / 4); i++) {
    ...
    public static Bitmap createBitmapWithBGR(byte[] data, int width, int height) {
        // bitmap pixels buffer
        byte[] bitmapData = new byte[width*height*4];
        // Copy pixels into place
        for (int i = 0; i < (bitmapData.length / 4); i++) {
            bitmapData[i * 4] = data[i * 3 + 2];        //R
            bitmapData[i * 4 + 1] = data[i * 3 + 1];    //G
            bitmapData[i * 4 + 2] = data[i * 3];        //B
            bitmapData[i * 4 + 3] = (byte) 0xff;        //A
        }
    ...
    public static Bitmap createBitmapWithRGBA(byte[] data, int width, int height) {
        Bitmap bitmap = Bitmap.createBitmap(width,height, Bitmap.Config.ARGB_8888);
